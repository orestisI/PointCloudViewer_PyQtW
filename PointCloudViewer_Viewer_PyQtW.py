...
class PointCloudViewer_Viewer_PyQtW(QWidget):
    def __init__(self,parent = None,streamMode = False,pointsPerThread = 4000,threadsNum = 4,red = 0,green = 255,blue = 0,backgroundColor = "grey"):
        super().__init__()
        self.parent = parent
    ...
    def EnableLiveStreamMode(self,enable):
        self.liveStreamMode = enable
    ...
    def StreamPoints(self,m):
        n = m.shape[1]
        self.aPointCloud = np.roll(self.aPointCloud,n,axis = 1)
        self.aPointCloud[:,0:n] = m
        self.update()
    ...
    def paintEvent(self,event):
        painter = QPainter()
        painter.begin(self)
        if len(self.projectionMatrixLst) != 0:
